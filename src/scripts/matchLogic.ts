...
export async function matchAnimation(player: string, computer: string) {
    ...
    const playerCard = document.querySelector("#playerCardChoose") as HTMLImageElement;
    const computerCard = document.querySelector("#computerCardChoose") as HTMLImageElement;
    ...
    playerCard.setAttribute("src", CARD_SRCS[player]);
    playerCard.setAttribute("alt", player);
    computerCard.setAttribute("src", CARD_SRCS[computer]);
    ...
    if (result === "player") {
        ...
        await playWinAnimation("#sprite2 img", "computerCharFolder", "computerChar");
...
async function playWinAnimation(selector: string, folder: string, prefix: string) {
    stopCharAnimation();
